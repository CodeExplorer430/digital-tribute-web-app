...
vi.mock('@/lib/server/media-consent', () => ({
  ...
  insertMemorialMediaConsent: (...args: unknown[]) =>
    mockInsertMemorialMediaConsent(...args),
}))
  ...
  createClient: async () => ({
    from: (table: string) => ({
      select: () => ({
        eq: () => ({
          single:
            table === 'site_settings' ? mockSiteSettingsSingle : mockPageSingle,
        }),
      }),
    }),
  }),
...
describe('POST /api/public/memorials/[slug]/media-consent', () => {
  ...
  it('sets a consent cookie and logs the event for protected memorials', async () => {
    ...
    expect(res.headers.get('set-cookie')).toContain(
      'everlume_memorial_media_consent_page-1=consent-token'
    )
  })
  ...
  it('returns not found when the memorial does not exist', async () => {
    ...
    const req = new Request(
      'http://localhost/api/public/memorials/missing/media-consent',
      { method: 'POST' }
    )
    const res = await POST(req as never, {
  ...
  it('returns a logging error when the consent event cannot be recorded', async () => {
    ...
    mockInsertMemorialMediaConsent.mockRejectedValue(
      new Error('Unable to record media consent.')
    )
