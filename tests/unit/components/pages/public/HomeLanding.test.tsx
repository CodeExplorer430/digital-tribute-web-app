...
import {
  HomeLanding,
  LandingContent,
} from '@/components/pages/public/HomeLanding'
...
describe('LandingContent', () => {
  it('renders primary CTA links with primary button styling', () => {
    ...
    expect(openAdminLinks[0].className).toContain('border-primary/80')
    expect(openAdminLinks[0].className).toContain('text-primary-foreground')
    expect(openAdminLinks[1].className).toContain('border-primary/80')
  ...
  it('renders memorial cards and fallback names when directory is enabled', () => {
    ...
    expect(
      screen.getByText(
        /Private and password-protected memorials remain outside this directory\./
      )
    ).toBeInTheDocument()
    expect(
      screen.getByRole('heading', { level: 4, name: 'Maria Santos' })
    ).toBeInTheDocument()
    expect(screen.getByText('Memorial')).toBeInTheDocument()
    expect(
      screen.getAllByRole('link', { name: 'Open memorial' })[0]
    ).toHaveAttribute('href', '/memorials/maria-santos')
  })

  it('renders empty-state text when directory is enabled with no memorials', () => {
    render(<LandingContent directoryEnabled memorials={[]} />)
    expect(
      screen.getByText('The public directory is ready.')
    ).toBeInTheDocument()
  })
})
