...
test.describe('real admin auth flow', () => {
  test.skip(
    ...
    'Runs only in the fake-auth Playwright lane.'
  )
  ...
  test('completes password setup and signs in with the new password', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole('link', { name: /continue to password reset/i })
    ).toBeVisible()
    await page
      .getByRole('link', { name: /continue to password reset/i })
      .click()
    ...
    await expect(
      page.getByText(/password updated\. redirecting to sign in/i)
    ).toBeVisible()
