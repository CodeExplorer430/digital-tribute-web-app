...
async function verifyCaptcha(
  ...
  const verifyUrl =
    process.env.CAPTCHA_VERIFY_URL?.trim() || DEFAULT_TURNSTILE_VERIFY_URL
...
export async function POST(request: NextRequest) {
  ...
  if (securityIssue) {
    return NextResponse.json(
      { code: 'CONFIGURATION_ERROR', message: securityIssue },
      { status: 503 }
    )
  }
  ...
  if (!captcha.ok) {
    const failureMessage =
      captcha.code === 'MISSING_INPUT'
        ...
        : captcha.code === 'UPSTREAM_ERROR' ||
            captcha.code === 'INVALID_RESPONSE'
  ...
  if (!memorial) {
    return NextResponse.json(
      { code: 'MEMORIAL_NOT_FOUND', message: 'Memorial not found.' },
      { status: 404 }
    )
  }
