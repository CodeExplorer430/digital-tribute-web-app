...
export async function GET(
  ...
  if (fixture) {
    ...
    if (!access.allowed) {
        ...
        return NextResponse.json(
          {
            code: 'FORBIDDEN',
            message:
              'This memorial must be unlocked before media can be viewed.',
          },
          { status: 403 }
        )
      }
      ...
      return NextResponse.json(
        { code: 'FORBIDDEN', message: 'This memorial is private.' },
        { status: 403 }
      )
    }
        ...
        return NextResponse.json(
          {
            ...
            message:
              'Confirm the protected media notice before viewing photos.',
          },
          { status: 403 }
        )
      }
    ...
    const resolvedPhotos = memorialRequiresProtectedMedia(fixture.memorial)
  ...
  const { data: page } = await supabase
    .from('pages')
    .select(
      'id, owner_id, privacy, access_mode, password_updated_at, media_consent_revoked_at'
    )
    .eq('slug', slug)
    .single()
  if (!page) {
    return NextResponse.json(
      { code: 'NOT_FOUND', message: 'Memorial not found.' },
      { status: 404 }
    )
  }
  ...
  if (error) {
    return NextResponse.json(
      { code: 'DATABASE_ERROR', message: 'Unable to load media.' },
      { status: 500 }
    )
  }

  const resolvedPhotos = memorialRequiresProtectedMedia(page)
    ? (photos ?? []).map((photo) => {
        const imageToken = createSignedMediaToken(photo.id, 'image')
        const thumbToken = createSignedMediaToken(photo.id, 'thumb')
        return {
