...
export async function POST(
  ...
  if (!parsedPayload.success) {
    return NextResponse.json(
      { code: 'VALIDATION_ERROR', message: 'Enter your access password.' },
      { status: 400 }
    )
  }

  const fixtureResult = verifyE2EMemorialPassword(
  ...
  if (fixtureResult) {
    ...
    const token = createMemorialAccessToken(
      fixtureResult.memorial.id,
      fixtureResult.memorial.password_updated_at || null
    )
    const response = NextResponse.json({ ok: true }, { status: 200 })

    response.cookies.set(
      getMemorialAccessCookieName(fixtureResult.memorial.id),
      token,
      {
        httpOnly: true,
  ...
  const passwordValid = verifyMemorialPassword(
    parsedPayload.data.password,
    page.password_hash
  )
  if (!passwordValid) {
