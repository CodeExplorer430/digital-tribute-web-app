...
export async function POST(
  ...
  if (!parsedParams.success) {
    return NextResponse.json(
      { code: 'VALIDATION_ERROR', message: 'Invalid memorial slug.' },
      { status: 400 }
    )
  }
  ...
  if (fixture) {
    ...
    if (!access.allowed) {
      return NextResponse.json(
        {
          code: 'FORBIDDEN',
          message: 'Unlock the memorial before viewing protected media.',
        },
        { status: 403 }
      )
    }
      ...
      return NextResponse.json(
        {
          ...
          message: 'Protected media consent is not required for this memorial.',
        },
        { status: 400 }
      )
    }
  ...
  const { data: memorial } = await supabase
    .from('pages')
    .select(
      'id, owner_id, slug, privacy, access_mode, password_updated_at, media_consent_revoked_at'
    )
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : 'Unable to record media consent.'
    return NextResponse.json(
      { code: 'CONSENT_LOG_ERROR', message },
      { status: 500 }
    )
  }
