...
export async function POST(_request: NextRequest, context: { params: Promise<{ jobId: string }> }) {
  ...
  if (!job.output_public_id) {
    return NextResponse.json({ code: 'MISSING_OUTPUT', message: 'Compressed output is missing.' }, { status: 409 })
  }
  ...
  const { error: updateError } = await supabase.from('video_upload_jobs').update({ status: 'attached' }).eq('id', job.id)
