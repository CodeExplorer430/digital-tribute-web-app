...
export async function POST(_request: NextRequest, context: { params: Promise<{ jobId: string }> }) {
  ...
  try {
    upstream = await fetch(`${getVideoTranscodeApiBaseOrThrow()}/jobs/${job.cloud_job_id || job.id}/start`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      {
        ...
        message: (upstreamPayload as { message?: string } | null)?.message || 'Unable to start transcode job.',
      },
      { status: 502 }
    )
  }

  const { error: updateError } = await supabase.from('video_upload_jobs').update({ status: 'processing' }).eq('id', job.id)
