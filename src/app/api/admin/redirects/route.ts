...
export async function GET() {
  ...
  if (error) {
    if (isSchemaMismatch(error)) {
      return NextResponse.json(
        { code: 'SCHEMA_MISMATCH', message: 'Database schema is outdated. Run the latest Supabase migrations.' },
        { status: 500 }
      )
    }
  ...
  const redirects: RedirectPayload[] = rows.map((item) => ({
    ...
    print_status: (item.print_status as 'unverified' | 'verified') ?? 'unverified',
...
export async function POST(request: NextRequest) {
  ...
  const redirectPayload: RedirectPayload = {
    ...
    print_status: ((insertData.print_status as 'unverified' | 'verified') ?? 'unverified'),
    last_verified_at: typeof insertData.last_verified_at === 'string' ? insertData.last_verified_at : null,
