...
export default async function RedirectFallbackPage({
  ...
  const reason = params.reason
    ? reasonText[params.reason] || reasonText.missing
    : reasonText.missing
  ...
  return (
    ...
    >
      ...
      <h1 className="text-3xl font-semibold tracking-tight text-foreground">
        We couldn&apos;t open this short link.
      </h1>
      <p className="mt-3 text-sm text-foreground/85">
        <span className="font-medium text-foreground">/{code}</span> is
        unavailable right now. {reason}
      </p>
      <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
        <Link
          href="/"
          className="inline-flex h-10 items-center rounded-md border border-border px-4 text-sm font-medium transition hover:bg-secondary"
        >
          Back to Home
        </Link>
        {directoryEnabled ? (
