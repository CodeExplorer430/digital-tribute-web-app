...
export function AuthFrame({
  ...
  return (
    <main
      id="main-content"
      className="page-shell min-h-screen px-4 py-8 md:py-12"
    >
      <div className="page-container grid gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(420px,0.95fr)] lg:items-stretch">
          ...
          <div className="relative flex h-full flex-col justify-between gap-10">
            <div className="space-y-5">
              ...
              <div className="space-y-4">
                ...
                <p className="max-w-md text-base leading-relaxed text-muted-foreground">
                  Everlume gives each family collaborator their own account so
                  publishing, moderation, and memorial upkeep stay secure and
                  accountable.
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-3">
              {trustPoints.map((point) => (
                <div
                  key={point}
                  className="rounded-2xl border border-border/75 bg-white/76 px-4 py-3 text-sm text-foreground shadow-[0_10px_24px_rgba(39,49,39,0.08)]"
                >
                  {point}
                </div>
              ))}
            ...
            <div className="rounded-3xl border border-border/70 bg-[linear-gradient(135deg,rgba(255,255,255,0.84),rgba(246,242,232,0.9))] p-5">
              <p className="text-sm font-medium text-foreground">
                Professional memorial operations
              </p>
              <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                Use separate passwords for each admin account. Invite
                collaborators by email and rotate credentials without disrupting
        ...
        <section className="surface-card auth-panel p-6 sm:p-8 md:p-10">
          <div className="mb-8 flex items-center justify-between gap-4">
            <Link
              href="/"
              className="inline-flex items-center gap-2 rounded-full border border-border/75 bg-[var(--surface-1)] px-3 py-2 text-sm font-medium text-foreground"
            >
