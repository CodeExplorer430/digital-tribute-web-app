...
export function ProtectedMediaConsentGate({
  slug,
  title = 'Media Viewing Notice',
  body,
  version = 1,
}: ProtectedMediaConsentGateProps) {
  ...
  const handleConsent = async () => {
    ...
    const response = await fetch(
      `/api/public/memorials/${slug}/media-consent`,
      {
        method: 'POST',
    ...
    if (!response.ok) {
      ...
      setErrorMessage(
        payload?.message || 'Unable to confirm media consent right now.'
      )
      setSubmitting(false)
      return
    }
  ...
  return (
    <section
      className="surface-card mx-auto max-w-3xl space-y-4 px-6 py-8 text-center md:px-10"
