    ...
    <div className="aspect-video overflow-hidden rounded-md bg-black">
      {video.provider === 'cloudinary' ? (
        <video
          ...
          src={buildCloudinaryVideoUrl(video.provider_id, {
            quality: 'auto:good',
            format: 'mp4',
          })}
        />
    ...
    <section id="videos" className="space-y-8 border-t border-border/80 pt-12">
      <div className="space-y-2 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          Recorded stories and moments from loved ones.
        </p>
      </div>
      {!videos || videos.length === 0 ? (
        <div className="surface-card mx-auto max-w-3xl px-6 py-10 text-center">
          ...
          <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
            When the family adds recorded tributes or memorial footage, they
            will appear here.
          </p>
        </div>
      ) : featured ? (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
          <article className="surface-card overflow-hidden p-3 md:p-4">
            ...
            {featured.title && (
              <p className="px-1 pt-3 text-sm font-medium text-muted-foreground">
                {featured.title}
              </p>
            )}
          </article>
            ...
            {remaining.length > 0 ? (
              remaining.map((video) => (
                ...
                >
                  ...
                  {video.title && (
                    <p className="px-1 pt-2 text-sm font-medium text-muted-foreground">
                      {video.title}
                    </p>
                  )}
                </article>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {videos.map((video) => (
            <article
              key={video.id}
              className="surface-card overflow-hidden p-3 md:p-4"
            >
              <VideoFrame video={video} />
              {video.title && (
                <p className="px-1 pt-3 text-sm font-medium text-muted-foreground">
