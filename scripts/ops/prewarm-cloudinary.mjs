...
if (!supabaseUrl || !serviceRoleKey || !cloudName) {
  console.error(
    'missing required env vars: NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SECRET_KEY (or SUPABASE_SERVICE_ROLE_KEY), NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME'
  )
  process.exit(1)
}
...
for (const photo of photos || []) {
  ...
  if (insertError) {
    console.error(
      `failed to write optimization run for photo ${photo.id}: ${insertError.message}`
    )
  }
}
